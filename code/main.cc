...
int main(){
    ...
    while(1){
        ...
        if(m_operacao == 1){
            ...
            fgets(nomevertice,100,stdin);
            nomevertice[strlen(nomevertice)-1]='\0';
            G = insere_vertice(G,nomevertice);
        }
            ...
            char p[100];
            double peso;
            printf("Digite o vertice de origem da aresta: ");
            ...
            p[strlen(p)-1]='\0';
            peso = atof(p);
            G = insere_aresta(G,vorig,vdest,peso);
        }
            ...
            fgets(nomevertice,100,stdin);
            nomevertice[strlen(nomevertice)-1]='\0';
            G = remove_vertice(G,nomevertice);
        }
        else if(m_operacao == 4){
            char vorig[100];
            char vdest[100];
            printf("Digite o vertice de origem da aresta a ser removida: ");
            fgets(vorig,100,stdin);
            vorig[strlen(vorig)-1]='\0';
            printf("Digite o vertice de destino da aresta a ser removida: ");
            fgets(vdest,100,stdin);
            vdest[strlen(vdest)-1]='\0';
            G = remove_aresta(G,vorig,vdest);
        }
        else if(m_operacao == 5){
            char vorig[100];
            char vdest[100];
            printf("Digite o vertice de origem do caminho: ");
            fgets(vorig,100,stdin);
            vorig[strlen(vorig)-1]='\0';
            printf("Digite o vertice de destino do caminho: ");
            fgets(vdest,100,stdin);
            vdest[strlen(vdest)-1]='\0';
